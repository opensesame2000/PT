package com.powertamil.lang.latin;

public class LChars {

    public static final char SPACE = ' ';
    public static final char SLASH = '/';
    public static final char DOT = '.';
    public static final char AT = '@';
    public static final char CARET = '^';
    public static final char COMMA = ',';
    public static final char COLON = ':';
    public static final char SEMI_COLON = ';';
    public static final char ASTERIX = '*';
    public static final char EQUALS = '=';
    public static final char PLUS = '+';
    public static final char MINUS = '-';
    public static final char UNDERSCORE = '_';
    public static final char AMPERSAND = '&';
    public static final char BACKSLASH = '\\';
    public static final char SMALLER_THAN = '<';
    public static final char LARGER_THAN = '>';
    public static final char EXCLAMATION_MARK = '!';
    public static final char QUESTION_MARK = '?';
    public static final char HASH = '#';
    public static final char PERCENT = '%';
    public static final char EUR = '€';
    public static final char GBP = '£';
    public static final char DOLLAR = '$';
    public static final char SINGLE_QUOTE = '\'';
    public static final char DOUBLE_QUOTE = '"';
    public static final char BRACKET_OPEN = '(';
    public static final char BRACKET_CLOSE = ')';
    public static final char SQUARE_BRACKET_OPEN = '[';
    public static final char SQUARE_BRACKET_CLOSE = ']';
    public static final char CURLY_BRACKET_OPEN = '{';
    public static final char CURLY_BRACKET_CLOSE = '}';
    public static final char PIPE = '|';
    public static final char NEWLINE = '\n';
    public static final char NUL = '\0';
    public static final String COMMA_SPACE = ", ";
    public static final String ENTITY_NBSP = "&nbsp;";
    public static final String NEWLINE_N = "\n";
    public static final String NEWLINE_R = "\r";
    public static final String EMPTY_STRING = "";
    public static final String SPACE_COMMA = " ,";
    public static final String SPACE_DOT = " .";
}
